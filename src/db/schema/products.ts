import { pgTable, varchar, integer, timestamp, text, decimal, uuid } from 'drizzle-orm/pg-core'
...
const productTable = pgTable('products', {
    ...
    ownerId: uuid().notNull().references(() => userTable.id, { onDelete: 'cascade' }),
...
export const insertProductSchema = createInsertSchema(productTable, {
    ...
    price: z.number().positive("El precio debe ser mayor a 0").transform((value) => parseFloat(value.toFixed(2))),
    stock: z.number().positive("El stock debe ser mayor a 0"),
    category: z.string().min(2, "La categoría debe tener al menos 2 caracteres").max(50),
});
