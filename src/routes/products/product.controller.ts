...
const getProduct = factory.createHandlers(async (c: Context) => {
    ...
    if (category) {
        const products = await db.select().from(productTable).where(eq(productTable.category, category))
    ...
    if (limitStr && pageStr) {
        ...
        const products = await db.select().from(productTable).limit(limit).offset(offset)
        return c.json({ products })
    }
...
const getProductById = factory.createHandlers(async (c: Context) => {
    ...
    const [product] = await db.select().from(productTable).where(eq(productTable.id, parseInt(id)))
