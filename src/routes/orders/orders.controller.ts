...
const createOrderController = factory.createHandlers(async (c: Context) => {
	try {
		const { products } = c.req.valid("json" as any) as {
		...
		if (!idempotencyKey)
			return c.json(
				{ message: "Idempotency-Key header is required", status: 400 },
				400,
			);
		...
		if (stockProducts.length !== products.length) {
			return c.json(
				{ message: "Some products do not exist", status: 404 },
				404,
			);
		}
...
const getAllOrdersController = factory.createHandlers(async (c: Context) => {
	try {
		...
		return c.json(
			{ message: "Orders retrieved successfully", status: 200, data: orders },
			200,
		);
	} catch (error) {
		console.log(error);
...
const getOrderByIdController = factory.createHandlers(async (c: Context) => {
	try {
		...
		return c.json(
			{ message: "Order retrieved successfully", status: 200, data: order },
			200,
		);
	} catch (error) {
		console.log(error);
...
const cancelOrderController = factory.createHandlers(async (c: Context) => {
    try {
        ...
        await db.update(orderTable).set({ status: "CANCELLED" }).where(eq(orderTable.id, id));

        return c.json({ message: "Order cancelled successfully", status: 200 }, 200);
    } catch (error) {
        console.log(error);
        return c.json({ message: "Internal server error", status: 500 }, 500);
    }
